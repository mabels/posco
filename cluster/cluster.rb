...
def load_certs(network)
  Dir.glob(File.join(get_config["certs"]["path"]||'/etc/letsencrypt/live',"*")).each do|dname|
    next unless File.directory?(dname)
      cacerts = network.cert_store.add_cacert(File.join(dname,'chain.pem')) +
            network.cert_store.add_cacert(File.join(dname,'fullchain.pem'))
      root_certs.each do |cpem|
        cacerts += network.cert_store.add_cacert(cpem)
      end
      pkg = network.cert_store.create_package(File.basename(dname),
        network.cert_store.add_private(File.join(dname,'privkey.pem')),
        network.cert_store.add_cert(File.join(dname,'cert.pem')),
        cacerts
       )
...
#binding.pry
  ...
  s = make_service(region, 'service' => Etcd::Service.new,
               ...
               'rndc_key'  => 'total geheim',
               'domains'   => nss.names,
               'ipv4_addr' => "#{ipv4.to_string.to_s}#SKIP_DNS",
               'ipv4_gw'   => j.ipv4_intern.to_s,
